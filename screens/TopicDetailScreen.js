  ...
  if (!topic) {
    return (
      <SafeAreaView style={s.safe}>
        <Text style={s.notFound}>주제를 찾을 수 없습니다.</Text>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={s.scroll}>
        <View style={s.categoryBadge}>
          <Text style={s.categoryText}>{categoryInfo?.icon} {categoryInfo?.title}</Text>
        </View>
        <Text style={s.name}>{topic.title}</Text>
        <Text style={s.era}>
          {topic.era}
          {topic.yearStart ? ` · ${topic.yearStart}${topic.yearEnd && topic.yearEnd !== topic.yearStart ? `~${topic.yearEnd}` : ''}년` : ''}
        </Text>
        {topic.importance ? <Text style={s.starsBig}>{starsFor(topic.importance)}</Text> : null}

        <View style={s.introRow}>
          <Text style={s.oneLiner}>{topic.oneLiner}</Text>
              ...
              <View key={stage.key}>
                ...
                <View style={s.blockHeadRow}>
                  <View style={s.stageLabelRow}>
                    <Text style={s.stageNumber}>{i + 1}</Text>
                    <Text style={s.blockTitle}>{stage.label}</Text>
                  </View>
                  <SegmentSpeaker segmentId={stage.key} text={text} activeSegment={activeSegment} onPlay={playSegment} />
                </View>
                <Text style={s.narrativeText}>{text}</Text>
              </View>
            );
          })
        ) : (
          ...
          <>
            ...
            <View style={s.chipRow}>
              {relatedKings.map((rk, i) => (
                ...
                >
                  <Text style={s.chipText}>👑 {rk.dynastyInfo?.title} · {rk.king.name}</Text>
                </TouchableOpacity>
              ))}
              {relatedTopics.map((rt) => (
                ...
                >
                  <Text style={s.chipText}>{TOPIC_CATEGORIES.find((c) => c.key === rt.category)?.icon} {rt.title}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </>
